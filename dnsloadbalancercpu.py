...
def load_check(feedback_freq, start_time):
    ...
    while True:
        ...
        configure_bind9(selected_server)
        plot_graph.close()
...
def configure_bind9(selected_server):
    global up_servers
    # print(up_servers[selected_server]['ip'])
    output = subprocess.Popen(
        "sed -i 's/.*192.168.*/abc.org.\ IN\ A\ " + up_servers[selected_server]['ip'] + "/' /etc/bind/forward.R1",
        stdout=subprocess.PIPE, shell=True)
    (out, err) = output.communicate()
    output = subprocess.Popen("service bind9 restart", stdout=subprocess.PIPE, shell=True)
    (out, err) = output.communicate()
